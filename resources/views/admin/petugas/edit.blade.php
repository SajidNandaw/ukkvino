...
<head>
<meta charset="UTF-8">
<title>Edit Petugas - KioStore</title>
...
/* SIDEBAR */
.sidebar{
width:220px;background:#fff;height:100vh;padding:20px;position:fixed;left:0;top:0;border-right:1px solid #e5e7eb;display:flex;flex-direction:column;
}
...
.menu ul li a{
text-decoration:none;color:#374151;padding:10px;display:block;border-radius:8px
}
...
.logout{
background:black;color:white;padding:10px;text-align:center;border-radius:8px;border:none;margin-top:auto;cursor:pointer;width:100%;
}
...
input,select{
width:100%;padding:12px;margin:8px 0 15px;border:1px solid #ddd;border-radius:8px;
}
...
<form method="POST" action="/admin/petugas/update/{{ $petugas->id }}">
@csrf

<label>Nama</label>
<input type="text" name="name" value="{{ $petugas->name }}" required>
...
<option value="nonaktif" {{ $petugas->status=='nonaktif' ? 'selected' : '' }}>
Nonaktif
</option>
